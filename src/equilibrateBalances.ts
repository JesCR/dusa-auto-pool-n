...
export async function equilibrateBalances(client: Client, account: IAccount, pair: PairV2, currentPrice: BigNumber) {
  ...
  if (balanceTokenAInUSD > balanceTokenBInUSD){
    //tengo que cambiar el exceso en dolares de WMAS a USDC
    //cuantos WMAS son el exceso en USD?
    const amountWMAStoSwapReal = excessValueInUSDReal / currentPriceWMASinUSDCReal;
    const amountWMAStoSwap = new BigNumber(amountWMAStoSwapReal * 10 ** tokenA.decimals);
    console.log(`${excessValueInUSDReal} $ in WMAS: ${amountWMAStoSwapReal}`);
    console.log(`{amountWMAStoSwap}: ${amountWMAStoSwapReal} * 10 ** ${tokenA.decimals} = ${amountWMAStoSwap}`);

    tmpResult = amountWMAStoSwap;
  } else {
    //tengo que cambiar el exceso en dolares de usdc a wmas
    //cuantos usdc son el exceso en USD?
    const amountUSDCtoSwapReal = excessValueInUSDReal;
    const amountUSDCtoSwap =  new BigNumber(amountUSDCtoSwapReal * 10 ** tokenB.decimals);
    console.log(`${excessValueInUSDReal} $ in WMAS: ${amountUSDCtoSwapReal}`);
    console.log(`{amountUSDCtoSwap}: ${amountUSDCtoSwapReal} * 10 ** ${tokenB.decimals} = ${amountUSDCtoSwap}`);

    tmpResult = amountUSDCtoSwap;
  } 
